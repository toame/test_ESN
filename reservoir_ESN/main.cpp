...
int main(void) {
	...
	for (int k = 1; k <= 20; k++) {
		...
		std::vector<std::vector<double>> A(210, std::vector<double>(210, 0.0));
		...
		output_learning.generate_simultaneous_linear_equationsA(A, output_node, 500, 1000, 100);
		output_learning.generate_simultaneous_linear_equationsb(b, output_node, teacher_signal, 500, 1000, 100);
		for (int i = 0; i < 1; i++) {
			for (int j = 0; j < 100; j++) {
				A[j][j] += 1e-12;
			}
			output_learning.IncompleteCholeskyDecomp2(A, L, d, 100);
			double eps = 1e-12;
			int itr = 100;
			output_learning.ICCGSolver(A, L, d, b, x, 100, itr, eps);
